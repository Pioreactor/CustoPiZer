...
if [ "$WORKER" == "1" ]; then
    ...
    if [ "$HEADLESS" == "1" ]; then
        ...
        echo "dtoverlay=vc4-kms-v3d,nohdmi" | sudo tee -a /boot/firmware/config.txt
        ...
        echo "enable_tvout=0" | sudo tee -a /boot/firmware/config.txt
        ...
        echo "hdmi_ignore_cec_init=1" | sudo tee -a /boot/firmware/config.txt
        echo "hdmi_ignore_cec=1" | sudo tee -a /boot/firmware/config.txt

        # disable POE, LDC probing
        echo "disable_poe_fan=1" | sudo tee -a /boot/firmware/config.txt
