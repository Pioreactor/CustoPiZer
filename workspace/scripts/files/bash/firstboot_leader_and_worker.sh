...
sudo -u $USERNAME "$PIO_VENV/bin/crudini" --ini-options=nospace --set $PIO_DIR/config.ini cluster.topology leader_hostname "$HOSTNAME"
sudo -u $USERNAME "$PIO_VENV/bin/crudini" --ini-options=nospace --set $PIO_DIR/config.ini cluster.topology leader_address "$HOSTNAME".local
...
sudo -u $USERNAME "$PIO_VENV/bin/crudini" --ini-options=nospace --set $PIO_DIR/config_"$HOSTNAME".ini cluster.topology leader_address 127.0.0.1
sudo -u $USERNAME "$PIO_VENV/bin/crudini" --ini-options=nospace --set $PIO_DIR/config_"$HOSTNAME".ini mqtt broker_address 127.0.0.1
